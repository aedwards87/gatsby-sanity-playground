...
const verifyInput = dayAndTime => {
  ...
  if (!opensAt) {
    return 'Choose when the location opens'
  }
  if (!closesAt) {
    return 'Choose when the location closes'
  }
  return opensAt < closesAt ? true : `Let's open the location before we close it on ${day}, shall we?`
}
